...
class HeatKernelConv(MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, K=5, t=3.0, method = 'Heat', kernel = 'Heat_I'):
        ...
        # Precompute coefficients
            ...
            lower = -.9
            upper = .9
            assert kernel in ['Heat_I', 'Heat_A', 'RWR_T', 'RWR_I', 'Beta_D','Random','High_A', 'High_I']
			...
			if kernel == 'High_I':
				...
				self.simple = False
                coeffs =  compare_fit_panelA(g_high_heat, 'Chebyshev', True, self.Threeterm,self.K+1, 0.001, 2.000,self.t) # Vadermonde indicator is True here
            elif kernel == 'RWR_T':
                alpha = self.t
                coeffs = [( alpha** k) for k in range(K + 1)]            
            elif kernel == 'Random':
